...
def apply_reweighing(
    ...
    """
    Retrain a classification model using reweighing.

    Supports both numeric labels (0/1) and categorical
    labels such as Yes/No.
    """

    # Calculate sample weights using the ORIGINAL target labels
    weights = calculate_reweighing_weights(
        sensitive_feature,
        y_train
    )

    # Train model with original labels and calculated weights
    model.fit(
        X_train,
        y_train,
